...
* Not supported in PostgresSQL and MySQL databases.
...
## Example:
Retun all customers that contains the pattern 'mer':
```sql
SELECT * FROM Customers
...
## Example:
Return all customers with City starting with "L", followed by any 3 characters, ending with "on":
```sql
SELECT * FROM Customers
...
## Example:
Return all customers starting with "b", "s", or "p":
```sql
SELECT * FROM Customers
