...
The following SQL selects all customers that starts with "a":
...
## Example:
Return all customers from a City that starts with 'type' followed by one wildcard character, then 'nd' and then two wildcard characters:
```sql
SELECT * FROM Customers
...
## Ends With
To return record that ends with a specific letter or phrase, add the % at the beginning of the letter or phrase.
...
Tip: You can also combine "start with" and "ends with":
...
## Contains
To return records that contains a specifiec letter or phrase, add the % both before and after the letter or phrase.
...
## Example:
Return all customers customers from Spain:
```sql
SELECT * FROM Customers
